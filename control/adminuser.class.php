...
class ADMINUSER {
	function __construct() {
		...
		if($this->checkUserNameExist() && ($this->doAction =='ADD_ADMINUSER'||$this->doAction =='UPDATE_ADMINUSER')) {
			if($this->rig_id) {
				$this->doAction = "EDIT";
			}
			else {
	...
	function  getRigInfo() {
		...
		WHERE 1 AND rl_type='A'";

		if(!empty($this->txtFilterName)) {
			$sqlSel .= " AND rl_username LIKE '%".$this->txtFilterName."%'";
		}
